...
func (s *spyStorage) Upload(_ context.Context, file *multipart.FileHeader, opts UploadOptions) (*UploadResult, error) {
	s.uploadedName = file.Filename
	s.uploadedOpts = opts
	return &UploadResult{Filename: file.Filename}, s.err
}

func (s *spyStorage) UploadFromReader(_ context.Context, reader io.Reader, filename string, opts UploadOptions) (*UploadResult, error) {
...
func (s *spyStorage) Delete(_ context.Context, path string) error {
	s.deletedPath = path
	return s.err
}

func (s *spyStorage) Exists(_ context.Context, path string) (bool, error) {
	s.checkedPath = path
	return s.exists, s.err
}

func (s *spyStorage) URL(_ context.Context, path string) (string, error) {
	s.urlPath = path
...
func TestManager_ForwardsToTheInjectedStorage(t *testing.T) {
	...
	// Act & Assert — upload
	result, err := manager.UploadFromReader(t.Context(), strings.NewReader("file body"), "photo.png", opts)
	...
	// Act & Assert — delete
	require.NoError(t, manager.Delete(t.Context(), "avatars/photo.png"))
	assert.Equal(t, "avatars/photo.png", spy.deletedPath)

	// Act & Assert — exists
	exists, err := manager.Exists(t.Context(), "avatars/photo.png")
	require.NoError(t, err)
	assert.True(t, exists)
	assert.Equal(t, "avatars/photo.png", spy.checkedPath)

	// Act & Assert — url and driver
	url, err := manager.URL(t.Context(), "avatars/photo.png")
	require.NoError(t, err)
...
func TestManager_PropagatesTheStorageError(t *testing.T) {
	...
	// Act & Assert
	_, err := manager.UploadFromReader(t.Context(), strings.NewReader("x"), "a.png", UploadOptions{})
	assert.Error(t, err)
	assert.Error(t, manager.Delete(t.Context(), "a.png"))

	_, err = manager.Exists(t.Context(), "a.png")
	assert.Error(t, err)

	_, err = manager.URL(t.Context(), "a.png")
	assert.Error(t, err)
}
...
func TestLocalStorage_UploadThenExistsThenDelete(t *testing.T) {
	...
	result, err := storage.UploadFromReader(t.Context(), bytes.NewBufferString("hello"), "note.txt", UploadOptions{Path: "docs/2026"})
	...
	// Act & Assert — exists
	exists, err := storage.Exists(t.Context(), "docs/2026/note.txt")
	require.NoError(t, err)
	assert.True(t, exists)

	missing, err := storage.Exists(t.Context(), "docs/2026/absent.txt")
	require.NoError(t, err)
	assert.False(t, missing, "a missing file is not an error, it is a false")

	// Act & Assert — delete
	require.NoError(t, storage.Delete(t.Context(), "docs/2026/note.txt"))
...
func TestLocalStorage_DeletingSomethingThatIsNotThereIsAnError(t *testing.T) {
	...
	assert.Error(t, storage.Delete(t.Context(), "never-existed.txt"))
}
...
func TestLocalStorage_URLRequiresABaseURL(t *testing.T) {
	...
	// Act & Assert
	url, err := withBase.URL(t.Context(), "/docs/note.txt")
	require.NoError(t, err)
	assert.Equal(t, "https://files.example.com/docs/note.txt", url)

	_, err = withoutBase.URL(t.Context(), "docs/note.txt")
...
func TestLocalStorage_EmptyPathUploadsToTheRoot(t *testing.T) {
	...
	result, err := storage.UploadFromReader(t.Context(), strings.NewReader("x"), "top.txt", UploadOptions{})
