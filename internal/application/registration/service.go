...
func (s *applicationService) Register(ctx context.Context, user *user.User) error {

	if err := s.checkExistingEmail(ctx, user.Email); err != nil {
		return fmt.Errorf("failed to register new store: %w", err)
	}

	role, err := s.roleRepo.GetRoleBySlug(ctx, role.DefaultRoleSlug)
	if err != nil {
	...
	return s.txManager.Do(ctx, func(txCtx context.Context) error {
		...
		user.HashPassword()
		createdUser, err := txUserRepo.CreateUser(txCtx, user)
		if err != nil {
