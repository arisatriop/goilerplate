...
func WireHandlers(app *bootstrap.App, useCases *UseCases, appServices *ApplicationServices, infrastructure *Infrastructure) *Handlers {
	...
	return &Handlers{
		...
		Foo:    handler.NewFoo(app.Validator, appServices.FooService, useCases.FooUC),
	}
}

// WireMiddleware creates all middleware components
func WireMiddleware(repos *Repositories, infrastructure *Infrastructure) *Middleware {
	// Create permission service for permission checking (with caching support)
	permissionService := auth.NewPermissionService(repos.AuthRepo, infrastructure.CacheService)

	return &Middleware{
		Auth:          middleware.NewAuth(infrastructure.JWTService, repos.AuthRepo, infrastructure.CacheService, permissionService),
