...
CREATE TABLE user_tokens (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), -- Unique identifier for the token
    ...
    token_type VARCHAR(50) NOT NULL, -- Type of token: email_verification, password_reset, email_change
    expires_at TIMESTAMP WITH TIME ZONE NOT NULL, -- When this token expires
    used_at TIMESTAMP WITH TIME ZONE DEFAULT NULL, -- When this token was used (NULL if not used yet)
    ip_address VARCHAR(45) DEFAULT NULL, -- IP address where token was created
    user_agent TEXT, -- User agent of the request that created the token
