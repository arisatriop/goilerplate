...
7. Post the comment with curl — Jira API v3 requires Atlassian Document Format (ADF) for the body. Build the ADF body by escaping `$COMMENT_TEXT` as a JSON string (use `jq -Rs .` to escape it), then embed it as a `text` node:
   ```bash
   ESCAPED=$(printf '%s' "$COMMENT_TEXT" | jq -Rs .)
   BODY=$(printf '{"body":{"version":1,"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":%s}]},{"type":"rule"},{"type":"paragraph","content":[{"type":"text","text":"Generated by "},{"type":"text","text":"Claude","marks":[{"type":"link","attrs":{"href":"https://claude.ai/code"}}]}]}]}}' "$ESCAPED")
